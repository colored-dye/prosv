...
class Arguments:
    ...
    benchmark_name: Literal["tinygsm8k"] = HfArg(default=None)
...
def main(args: Arguments):
    ...
    for concept_id, metarec in metadata.items():
        ...
        save_path = dump_dir / f"eval_{args.benchmark_name}.parquet"

        if save_path.exists():
            logger.warning(f"Already done concept id {concept_id}; skipping")
            continue
        
        steered_generations_path = dump_dir / f"steered_benchmark_{args.benchmark_name}.parquet"
