...
def get_test_data(args: Arguments, tokenizer: PreTrainedTokenizer, concept_id: int):
    ...
    # if any(m.lower() in args.model_path.lower() for m in HAS_SYSTEM_PROMPT_MODELS):
    #     logger.warning("Model uses system prompt; using default system prompt.")
    #     inputs = [
    #         tokenizer.decode(
        ...
        for factor_lr in args.factor_learning_rate_list:
            for factor_init in args.factor_init_scale_list:
                ...
                for concept_id, metarec in metadata.items():
                    ...
                    test_prompts, test_dataloader = get_test_data(args, tokenizer, concept_id)
