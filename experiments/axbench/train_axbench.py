...
class Arguments:
    ...
    adapter_type: str = HfArg(default="add_free")
    positions: str = HfArg(default="f4")
    optimizer: Literal["adam", "sgd"] = HfArg(default="adam")
    objective: Literal["lang", "simpo"] = HfArg(default="lang")
    ...
    epochs: int = HfArg(default=None)
    vector_learning_rate: float = HfArg(default=None)
    vector_init_scale: float = HfArg(default=None)
    factor_learning_rate: float = HfArg(default=None)
    factor_init_scale: float = HfArg(default=None)
    ...
    for epoch_i in pgbar_epoch:
        ...
        for step, batch in enumerate(pgbar_step):
            if args.objective == "lang":
                locations = [
                    loc[loc != -1].tolist() for loc in batch["intervention_locations"]
                ]
                ...
                locations = [
                    loc[loc != -1].tolist() for loc in batch_processed["intervention_locations"]
                ]
...
def get_concept_dataloader(
    ...
    if args.objective == "lang":
        ...
        for x in dataset:
            ...
            if (tokenizer.bos_token is not None) and (input_ids[0] == tokenizer.bos_token_id):
    ...
    if args.objective == "lang":
        ...
        logger.warning("Winning: **" + tokenizer.decode(train_set[0]["positive_input_ids"]) + "**")
        logger.warning("Losing: **" + tokenizer.decode(train_set[0]["negative_input_ids"]) + "**")
...
def main(args: Arguments):
    ...
    factor_init = args.factor_init_scale
    vector_lr = args.vector_learning_rate
    vector_init = args.vector_init_scale

    logger.warning("============================================================================")
    logger.warning(f"B: {batch_size} || E: {epochs} || F_lr: {factor_lr} || F_init: {factor_init} || V_lr: {vector_lr} || V_init: {vector_init}")
    logger.warning("============================================================================")

    output_base_dir = args.output_dir
    args.output_dir = Path(output_base_dir)
    ...
    for concept_id, metarec in metadata.items():
        ...
        train_sv(
            ...
            epochs=epochs,
            vector_learning_rate=vector_lr,
            vector_init_scale=vector_init,
