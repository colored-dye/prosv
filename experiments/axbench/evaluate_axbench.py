...
class Arguments:
    ...
    max_new_tokens: int = HfArg(default=None)
...
def main(args: Arguments):
    ...
    if args.max_new_tokens is not None:
        _, tokenizer = load_hf_model_tokenizer(
    ...
    for concept_id, metarec in metadata.items():
        ...
        if args.max_new_tokens is not None:
            steer_df["steered_generation"] = [
                tokenizer.decode(
                    tokenizer.encode(s, add_special_tokens=False)[: args.max_new_tokens]
                )
                for s in steer_df["steered_generation"].tolist()
            ]
