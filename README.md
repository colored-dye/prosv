...
We have provided a **Jupyter noteboook** in `experiments/concept/concept.ipynb` for PrOSV with `p2+s2` interventions, i.e. two prompt prefix tokens and two prompt suffix tokens, as well as *full-sequence SV (FSSV)*, along with both their trained **checkpoints**.
